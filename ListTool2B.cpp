...
Element::Element()       { elementType = 'B'; }   //  Sets element type.
...
void Element::display()         //  Virtual.  Displayed only if function is
		     ...
		     {  cout << "\nLISTTOOL - WARNING:\n\t'Display()' not "
			         << "implemented inside derived class.";  }
								...
								//  Class "NumElement" member functions:
...
int NumElement::compare(Element* el)   //  Virtual. Compares own number with  el's.
...
void TextElement::strcpy2(char* s, const char* t)    //  Home made "strcpy"
	  {  while (*t)  *s++ = *t++;   *s = '\0';  }
...
TextElement::~TextElement()                     //  Deletes the text ID.
      {  delete [] text;   }

int TextElement::compare(Element* el)   // Virtual. Compares own name/text with el's.
...
Element* List::remove()  {     //    element in a LIFO-/FIFO-list:
  ...
  if (liType == LIFO  ||  liType == FIFO)  {   // Legal list type:
     if (!isEmpty())  {                 //  Something in the list:
        ...
        if (first == nullptr) last = nullptr; //  The list now are empty.
